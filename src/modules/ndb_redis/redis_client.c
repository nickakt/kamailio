...
err2:
		...
		if(init_without_redis == 1) {
			LM_WARN("failed to initialize redis server [%.*s], but continuing"
					" with other servers (init_without_redis=1). ctxRedis=%p\n",
					rsrv->sname->len, rsrv->sname->s, rsrv->ctxRedis);
			redisFree(rsrv->ctxRedis);
			rsrv->ctxRedis = NULL;
			LM_INFO("proceeding to next redis server in list...\n");
			continue;
		}
		return -1;
...
err:
		...
		if(init_without_redis == 1) {
			LM_WARN("failed to connect to redis server [%.*s], but continuing"
					" with other servers (init_without_redis=1). ctxRedis=%p\n",
					rsrv->sname->len, rsrv->sname->s, rsrv->ctxRedis);
			if(rsrv->ctxRedis) {
				redisFree(rsrv->ctxRedis);
